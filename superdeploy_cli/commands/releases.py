...
def releases(app, limit):
    ...
    inspect_cmd = f"""
    docker inspect superdeploy-{app}-1 2>/dev/null | jq -r '.[0] | {{
        image: .Config.Image,
        created: .Created,
    ...
    try:
        ...
        # Parse JSON
        current_data = json.loads(current_info) if current_info.strip() != '{}' else {}
        ...
        if current_data:
            ...
            image = current_data.get("image", "unknown")
            ...
            git_sha = labels.get("com.superdeploy.git.sha", "unknown")[:7]
...
def rollback(app, target, force):
    ...
    # Confirm
    if not force:
        if not Confirm.ask(f"Continue with rollback?"):
            console.print("[yellow]⏹️  Rollback cancelled[/yellow]")
            raise SystemExit(0)
    
    # Trigger deployment via Forgejo API
    console.print(f"[cyan]🔄 Triggering rollback deployment...[/cyan]")
    ...
    try:
        ...
        if response.status_code == 204:
            console.print(f"[green]✅ Rollback triggered successfully![/green]")
            console.print(
