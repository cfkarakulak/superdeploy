...
def verify_password(env_vars):
    ...
    # Create a simple hash from system username + hostname
    expected_hash = hashlib.sha256(
        f"{os.getenv('USER', 'unknown')}:{os.uname().nodename}".encode()
    ).hexdigest()[:16]
    
    console.print(
        Panel(
            "[yellow]⚠️  Sensitive data access[/yellow]\n\n"
            "[white]Enter verification code:[/white]",
            border_style="yellow",
        )
    )
    ...
    # Simple verification - just check it's not empty and matches expected pattern
    user_hash = hashlib.sha256(password.encode()).hexdigest()[:16]
    
    # For security, we don't store the password, just verify user is intentional
    if len(password) < 4:
...
@click.option("--no-mask", is_flag=True, help="Show full values (requires verification)")
def env_show(show_all, app, no_mask):
    ...
    # Categorize vars
    infra_keys = [
        "GCP_PROJECT_ID", "GCP_REGION", "SSH_KEY_PATH", "SSH_USER",
        "CORE_EXTERNAL_IP", "CORE_INTERNAL_IP", "SCRAPE_EXTERNAL_IP", 
        "SCRAPE_INTERNAL_IP", "PROXY_EXTERNAL_IP", "PROXY_INTERNAL_IP",
        "FORGEJO_ADMIN_USER", "FORGEJO_ADMIN_PASSWORD", "FORGEJO_ADMIN_EMAIL",
        "FORGEJO_ORG", "FORGEJO_PAT", "REPO_SUPERDEPLOY",
        "DOCKER_USERNAME", "DOCKER_TOKEN", "GITHUB_TOKEN",
        "VM_MACHINE_TYPE", "VM_DISK_SIZE", "VM_IMAGE"
    ]
    
    app_keys = [
        "POSTGRES_USER", "POSTGRES_PASSWORD", "POSTGRES_DB",
        "RABBITMQ_USER", "RABBITMQ_PASSWORD",
        "REDIS_PASSWORD", "API_SECRET_KEY",
        "GRAFANA_ADMIN_USER", "GRAFANA_ADMIN_PASSWORD",
        "ALERT_EMAIL", "SENTRY_DSN"
    ]
...
def env_check():
    ...
    console.print(Panel("[bold cyan]🔍 Environment Health Check[/bold cyan]", expand=False))
    ...
    # Check required vars
    required = [
        "GCP_PROJECT_ID", "GCP_REGION", "SSH_KEY_PATH",
        "DOCKER_USERNAME", "GITHUB_TOKEN",
        "POSTGRES_PASSWORD", "RABBITMQ_PASSWORD", "API_SECRET_KEY"
    ]
