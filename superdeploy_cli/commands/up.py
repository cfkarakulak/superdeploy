...
from superdeploy_cli.utils import load_env, run_command, get_project_root, validate_env_vars
...
def update_ips_in_env(project_root, env_file_path):
    ...
    try:
        ...
        console.print(f"[green]✅ Updated IPs:[/green]")
...
"""
    ...
    console.print(f"[green]✅ Ansible inventory generated[/green]")
...
def clean_ssh_known_hosts(env):
    ...
    for ip in [env.get('CORE_EXTERNAL_IP'), env.get('SCRAPE_EXTERNAL_IP'), env.get('PROXY_EXTERNAL_IP')]:
        if ip:
...
def up(skip_terraform, skip_ansible, skip_git_push):
    ...
    with Progress(
        ...
        if not skip_terraform:
            ...
            # Wait for VMs
            task_wait = progress.add_task("[yellow]Waiting for VMs to be ready (120s)...", total=120)
            for i in range(120):
                time.sleep(1)
        ...
        # Ansible
        if not skip_ansible:
            task2 = progress.add_task("[cyan]Configuring services (Ansible)...", total=1)
...
"""
        ...
        if not skip_git_push:
            ...
            try:
                subprocess.run(['git', 'remote', 'remove', 'forgejo'], capture_output=True)
            except:
                pass
    ...
    console.print(f"[cyan]👤 Login:[/cyan]   {env.get('FORGEJO_ADMIN_USER', 'admin')} / {env.get('FORGEJO_ADMIN_PASSWORD', '')}")
