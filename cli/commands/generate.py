...
def generate(project):
    ...
    try:
        ...
        console.print(
            f"[dim]✓ Loaded {len(addons)} addon(s): {', '.join(addons.keys())}[/dim]"
        )
...
"""
        ...
        console.print(f"  [green]✓[/green] Generated: {env_file}")
        ...
        console.print("  [yellow]⚠[/yellow] Remember to edit .env and add your tokens!")
    ...
    console.print(
        f"  [green]✓[/green] Generated: {compose_dir}/docker-compose.core.yml"
    )

    # Generate docker-compose.apps.yml
    generate_docker_compose_apps(config, compose_dir)
    console.print(
        f"  [green]✓[/green] Generated: {compose_dir}/docker-compose.apps.yml"
    )
    ...
    steps:
      ...
      - name: Deploy with zero-downtime
        run: |
          ...
          cat docker-compose-{app_name}.yml | \
            sed "s/container_name: $CONTAINER_NAME/container_name: $NEW_CONTAINER/" | \
            awk -v port="{port}" -v temp="$TEMP_PORT" '{{
              if ($0 ~ /- "?'"{port}:{port}"'/) {{
                gsub(/{port}:{port}/, temp":"port)
              }}
              print
            }}' > /tmp/docker-compose-new-{app_name}.yml
          ...
          # SIMPLE APPROACH: Stop old, start new (minimal downtime)
