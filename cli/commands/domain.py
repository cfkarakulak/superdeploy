...
def add_domain(project: str, app_name: str, domain: str):
    ...
    try:
        ...
        if not config_file.exists():
            console.print(f"[red]✗ Project '{project}' not found at {config_file}[/red]")
            raise click.Abort()
        ...
        if app_name not in apps:
            console.print(f"[red]✗ App '{app_name}' not found in project '{project}'[/red]")
        ...
        try:
            ...
            for vm_name, vm_data in vms.items():
                if vm_name.startswith(f"{project}-{vm_role}"):
        ...
        if not click.confirm(f"Have you added the DNS record for {domain}?", default=False):
            console.print("Aborted. Add the DNS record and try again.")
            raise click.Abort()
        
        # Update project.yml
        console.print(f"Updating project.yml...")
        ...
        console.print(f"This will update Caddyfile and reload Caddy...")
        
        # Run superdeploy up with caddy addon only
        result = subprocess.run(
            ["superdeploy", "up", "-p", project, "--skip-terraform", "--addon", "caddy"],
            cwd=Path.cwd()
        )
        
        if result.returncode != 0:
            console.print(f"[red]✗ Failed to redeploy Caddy[/red]")
            raise click.Abort()
...
def list_domains(project: str):
    ...
    try:
        ...
        console.print(f"[dim]Total apps: {total_apps} | With domains: {apps_with_domains}[/dim]")
        console.print()
...
def remove_domain(project: str, app_name: str):
    ...
    try:
        ...
        # Confirm removal
        if not click.confirm(f"Remove domain '{old_domain}' from {app_name}?", default=False):
        ...
        console.print(f"App now accessible via port-based routing only")
