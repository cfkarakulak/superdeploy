...
class TemplateMerger:
    ...
    def merge_compose(self, addons: Dict[str, Addon], project_config: dict) -> str:
        ...
        network_subnet = project_config.get('network', {}).get('subnet', '172.20.0.0/24')
        ...
        for addon_name, addon in addons.items():
            ...
            addon_config = (
                project_config.get('infrastructure', {}).get(addon_name, {}) or
                project_config.get('core_services', {}).get(addon_name, {})
            )
    ...
    def merge_ansible_tasks(self, addons: Dict[str, Addon], project_config: dict) -> List[dict]:
        """
        ...
        for addon_name, addon in addons.items():
            ...
            tasks = addon.ansible_tasks.copy() if isinstance(addon.ansible_tasks, list) else []
            
            # Substitute variables in tasks
            for task in tasks:
                task = self._substitute_ansible_vars(task, {
                    'addon_name': addon_name,
                    'project_name': project_name,
                    'version': addon_config.get('version', addon.get_version()),
    ...
    def _substitute_ansible_vars(self, task: dict, context: dict) -> dict:
        ...
        if isinstance(task, dict):
            return {k: self._substitute_ansible_vars(v, context) for k, v in task.items()}
