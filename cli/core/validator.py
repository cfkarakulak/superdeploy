...
class ValidationEngine:
    ...
    def _validate_infrastructure_config(
        ...
        if addons is not None and not isinstance(addons, dict):
            errors.append(
                ValidationError(
                    error_type="invalid_config", message="'addons' must be a dictionary"
                )
            )
            return errors
    ...
    def _validate_vm_config(self, project_config: dict) -> List[ValidationError]:
        """
    ...
    def _validate_addon_names(
        self, project_config: dict, available_addons: Optional[Set[str]] = None
    ) -> List[ValidationError]:
        """
    ...
    def _validate_addon_configs(self, project_config: dict) -> List[ValidationError]:
        """
    ...
    def _validate_subnet_conflicts(
        self, project_config: dict, project_name: str
    ) -> List[ValidationError]:
        """
        ...
        for existing_subnet_str, existing_project in used_subnets.items():
            ...
            try:
                existing_subnet = ipaddress.ip_network(
                    existing_subnet_str, strict=False
                )
    ...
    def _validate_addon_dependencies(
            ...
            if config_file.exists():
                try:
                    ...
                    if config:
                        ...
                        subnet = network_config.get(
                            "docker_subnet"
                        ) or network_config.get("subnet")
