...
class AddonLoader:
    ...
    def load_addon(self, addon_name: str) -> Addon:
        ...
        if not addon_path.exists() or not addon_path.is_dir():
            raise AddonNotFoundError(
                f"Addon '{addon_name}' not found at {addon_path}"
            )
        ...
        try:
            ...
            raise AddonValidationError(
                f"Addon '{addon_name}' has invalid YAML: {e}"
            )
    ...
    def discover_available_addons(self) -> List[str]:
        """
        ...
        for item in self.addons_dir.iterdir():
            if item.is_dir() and (item / "addon.yml").exists():
